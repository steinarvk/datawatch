...
class FetcherLoop(object):
    def __init__(self, on_fetched, user_agent, target_link_filter, fetching_ratelimit=0.2, discovery_delay=300, fetch_delay=60, verbose=False):
    ...
    def _run_target_fetch(self, task):
        url = task.payload
        resp = self._geturl(url, allow_failure=True)
        self._on_fetched(url, resp, resp.content)
    ...
    def _add_target(self, url):
        ...
        self._loop.schedule_task(
            callback=self._run_target_fetch,
            payload=url,
